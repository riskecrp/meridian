...
export async function addWeapon(data) {
  ...
  if (data.ammoNames && data.ammoNames.length > 0) {
    data.ammoNames.forEach(a => {
      ...
      run("INSERT OR IGNORE INTO weapon_ammo_compat (weapon_id, ammo_name, ammo_type) VALUES (?, ?, ?)",
        [r.lastInsertRowid, a, atype]);
    });
  }
  logAudit(actor.id, actor.name, 'CREATE', 'weapon_ammo', null, data.name, 'Loadout added');
  return { ok: true };
}
...
export async function addAttachmentCompat(weaponId, attachmentName) {
  const actor = await requireActor(3);
  run("INSERT OR IGNORE INTO weapon_attachments (weapon_id, attachment_name) VALUES (?, ?)",
    [weaponId, attachmentName]);
